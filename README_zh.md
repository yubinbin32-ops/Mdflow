...
| 评估维度 | 传统纯 Markdown 文档 | `context-mode` / CLI 封装 | `code-context-engine` | **mdflow** |
| :--- | :---: | :---: | :---: | :---: |
| **上下文粒度** | 粗粒度（整个文件） | 会话级提示词拼接 | 代码 AST 语法树索引 | **任务级精准语义切片** |
| **上下文节省** | 0%（全量冗余） | 较低 | ~20% | **70% – 90%** |
| **Git 原子撤回** | 容易遗漏或冲突 | 无（单次生效） | 需重新构建索引 | **100% 零漂移（纯文本 `graph.json`）** |
| **验收证据** | 人工口头说明 | 无 | 无 | **密码级 Checkpoint 验证门禁** |
| **全局可视化** | 无 | 无 | 无 | **原生交互式 Canvas 画布** |
| **编辑器支持** | 手动复制粘贴 | 单独命令行 | 自定义脚本 | **标准 MCP（通用各主流 AI 工具）** |

### 真实工程实测基准（以 mdflow 自身 27 Blocks、6 Chains、63 Checkpoints 为例）

| 上下文维护模式 | 每次 Prompt 加载体积 | 多轮对话准确率与召回 | Git Discard 撤回表现 |
| :--- | :---: | :---: | :---: |
| **传统全量 Markdown 文档**（架构+计划+规范多文件） | ~9,318 tokens（32,614 字符） | 极易多轮失真、遗忘约束 | ❌ 外部数据库/代码脱节 |
| **mdflow 任务动态切片**（`context_for_task`） | **~1,015 tokens（3,553 字符）** | **100% 目标事实召回，0 失真** | **✅ 100% 原子同步回退** |
| **净收益对比** | **节省 89.1% 上下文消耗** | **消除幻觉与漂移** | **架构与代码绝对零漂移** |
